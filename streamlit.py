...
### OUTPUT
image_cover = Image.open('E:\github\streamlit-thesis\images\ML.jpeg')
st.image(image_cover,use_column_width= True)

st.write("""
# Predict Total Sale Next Month From Ensemble Learning
...
#Load model
device = 'cuda'
model_cat,model_xgb,model_rf,model_lr,model_knn,meta_model = load_model()

lr_scaler,knn_scaler = Scale(lr_train,knn_train)

if st.button('Show result training'):
       with st.spinner(" ⏳ Processing training... this may take awhile! \n Don't stop it!"):
              ...
              mae = mean_absolute_error(y_train, catboost_train_pred)
              st.write("- Train MAE: ", mae)
              mae = mean_absolute_error(y_val, catboost_val_pred)
              st.write("- Validation MAE:  ", mae)
              mape = mean_absolute_percentage_error(catboost_train_pred, y_train)
              st.write("- Train MAPE:  ", mape)
              mape = mean_absolute_percentage_error(catboost_val_pred,y_val)
              st.write("- Validation MAPE:  ", mape)
              ...
              st.write("- Validation MAE:  ", mae)
              mape = mean_absolute_percentage_error( xgb_train_pred, y_train)
              st.write("- Train MAPE:  ", mape)
              ...
              st.write("- Validation MAE:  ", mae )
              mape = mean_absolute_percentage_error( rf_train_pred, y_train)
              st.write("- Train MAPE:  ", mape)
              ...
              st.write("- Validation MAE:  ", mae)
              mape = mean_absolute_percentage_error(lr_train_pred, y_train)
              st.write("- Train MAPE:  ", mape)
              ...
              st.write("- Validation MAE:  ", mae)
              mape = mean_absolute_percentage_error(knn_train_pred, y_train)
              st.write("- Train MAPE:  ", mape)
...
st.write("#### Test set for the ensemble model:")
if st.button('Show result',key='a'):
...
st.write("#### Final predict 13th month of an item:")
if st.button('Show result',key='b'):
       with st.spinner(" ⏳ Waiting ... this may take awhile! \n Don't stop it!"):
              with open("rsfn", "rb") as fp:
                     bf = pickle.load(fp)
       bf[df_cat.index[(df_cat['shop_id']==shop_id) & (df_cat['item_id']==item_id)][0]]
       st.write("#### Final result = Final predict x Total number of product last month")           
